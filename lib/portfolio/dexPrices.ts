...
function bestPriceUsd(pairs: any, target: string): number | null {
...
async function fetchOne(addr: string): Promise<number | null> {
  ...
  try {
    ...
    if (res.ok && (res.headers.get('content-type') || '').includes('application/json')) {
      ...
      price = bestPriceUsd(data?.pairs, key);
    }
  } catch {
    /* leave null */
  } finally {
...
// Returns address → priceUsd for the addresses that have a price (others omitted).
export async function fetchUsdPrices(addresses: string[]): Promise<Map<string, number>> {
  const uniq = [
    ...new Set(
  ...
  for (let i = 0; i < uniq.length; i += CONCURRENCY) {
    ...
    const prices = await Promise.all(slice.map(fetchOne));
    slice.forEach((a, j) => {
      const p = prices[j];
