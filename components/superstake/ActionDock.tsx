...
// at every scroll position instead. On a phone it spreads into a bar, riding
// above the app's bottom nav so the button lands under a thumb without burying
// the navigation.
//
...
export default function ActionDock({ token }: ActionDockProps) {
  ...
  return (
    <>
      {/* ── phone: a bar sitting on top of the app's bottom nav ──
          `MobileBottomNav` is `fixed bottom-0` at `h-16`, so anchoring here to
          bottom-0 buried it. This clears the nav's 64px and its safe-area
          inset, then lifts 8px more so the two don't read as one bar. */}
      <div
        className="fixed inset-x-0 z-[60] flex items-center justify-between gap-2 border-t border-[var(--line)] px-2.5 py-1.5 md:hidden"
        style={{
          bottom: 'calc(4rem + env(safe-area-inset-bottom) + 8px)',
          background:
            'linear-gradient(180deg,color-mix(in srgb,var(--app-bg) 55%,transparent),var(--app-bg) 45%)',
        }}
      >
        <Links token={token} />
        <Buy href={buyHref} />
      ...
      <div
        className="group fixed bottom-[41px] right-4 z-[60] hidden items-center gap-1.5 rounded-full border border-[var(--line-strong)] bg-[color-mix(in_srgb,var(--panel)_92%,transparent)] p-1 shadow-xl shadow-black/50 backdrop-blur md:flex"
        onFocus={() => setTight(false)}
      >
        <span
          className={`flex items-center overflow-hidden transition-[max-width,opacity] duration-300 group-hover:max-w-[120px] group-hover:opacity-100 group-focus-within:max-w-[120px] group-focus-within:opacity-100 motion-reduce:transition-none ${
            tight ? 'max-w-0 opacity-0' : 'max-w-[120px] opacity-100'
          }`}
        >
          <Links token={token} />
          <span aria-hidden className="my-0.5 ml-1.5 w-px self-stretch bg-[var(--line)]" />
        </span>
        <Buy href={buyHref} />
      </div>
    </>
...
function Links({ token }: { token: string }) {
  const cls =
    'grid h-6 w-6 place-items-center rounded-full border border-transparent text-[var(--text-muted)] transition-colors hover:border-[var(--line)] hover:bg-[var(--surface)] hover:text-[var(--text)] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[var(--line-strong)]';
  return (
...
function Buy({ href }: { href: string }) {
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="inline-flex items-center gap-1 rounded-full px-3 py-1.5 text-[11.5px] font-bold tracking-[-0.01em] text-white transition-transform hover:scale-[1.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/40 motion-reduce:transition-none motion-reduce:hover:scale-100"
