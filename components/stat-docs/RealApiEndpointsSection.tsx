...
export default function RealApiEndpointsSection({ statId, tokenAddress }: RealApiEndpointsSectionProps) {
  ...
  if (!mapping) {
    return (
      <div className="space-y-4">
        <h2 className="text-xl font-semibold text-[var(--text)]">API Endpoints</h2>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-[var(--text)] mb-2">Real API Endpoints</h2>
      ...
      {mapping.dataProcessing && (
        <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-4">
          ...
          <p className="text-sm text-[var(--text-muted)] ml-7">{mapping.dataProcessing}</p>
        </div>
      )}
      ...
      <div className="space-y-4">
        {mapping.endpoints.map((endpoint, index) => {
          ...
          return (
            <div key={index} className="bg-[var(--panel)] border border-[var(--line)] rounded-xl overflow-hidden">
              {/* Endpoint Header */}
              <div className="p-4 border-b border-[var(--line)]">
                <div className="flex items-start justify-between gap-4 mb-2">
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-[var(--text-muted)]">{endpoint.description}</p>
                  </div>
                  <button
                    onClick={() => copyEndpoint(fullUrl)}
                    className="text-xs px-3 py-1.5 bg-[var(--panel)] hover:bg-[var(--surface-2)] text-[var(--text)] border border-[var(--line)] rounded transition-colors flex-shrink-0"
                  >
                    📋 Copy URL
                  </button>
                </div>
                ...
                {/* URL Display */}
                <div className="mt-3 bg-[var(--app-bg)] border border-[var(--line)] rounded p-3">
              ...
              {endpoint.parameters && Object.keys(endpoint.parameters).length > 0 && (
                <div className="p-4 border-b border-[var(--line)] bg-[var(--app-bg)]">
                  <h4 className="text-sm font-semibold text-[var(--text)] mb-3">Parameters</h4>
                      ...
                      <div key={key} className="flex items-start gap-3 text-xs">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="px-1.5 py-0.5 bg-[var(--panel)] border border-[var(--line)] rounded text-purple-400">
                              {param.type}
                            </span>
                            {param.required && (
                          ...
                          <p className="text-[var(--text-muted)]">{param.description}</p>
                          <p className="text-[var(--text-muted)] mt-1">Example: <code className="text-purple-400">{param.example}</code></p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {endpoint.sampleResponse && (
                <div className="p-4">
                  <button
                    ...
                    className="w-full flex items-center justify-between text-sm font-semibold text-[var(--text)] hover:text-purple-400 transition-colors mb-3"
                  >
                  ...
                  {isExpanded && (
                    <div className="bg-[var(--app-bg)] border border-[var(--line)] rounded overflow-hidden">
                      <pre className="p-4 overflow-x-auto text-xs text-[var(--text-muted)] font-mono max-h-96 overflow-y-auto">
      ...
      {mapping.endpoints.length > 1 && (
        <div className="bg-gradient-to-r from-purple-500/10 to-blue-500/10 border border-purple-500/30 rounded-lg p-6">
          <h3 className="text-lg font-semibold text-[var(--text)] mb-4">API Call Flow</h3>
          <div className="space-y-3">
            {mapping.endpoints.map((endpoint, index) => (
              <div key={index} className="flex items-start gap-3">
                <div className="flex-shrink-0 w-8 h-8 rounded-full bg-purple-600 flex items-center justify-center text-[var(--text)] font-bold text-sm">
                  {index + 1}
                </div>
