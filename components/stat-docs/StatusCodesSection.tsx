...
export default function StatusCodesSection() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-[var(--text)]">HTTP Status Codes</h2>
      
      <div className="space-y-3">
        {statusCodes.map(status => (
          <div
            key={status.code}
            className="bg-[var(--panel)] border border-[var(--line)] rounded-lg p-4 flex items-start gap-4"
          >
            <span className={`px-3 py-1 text-sm font-bold rounded border ${getStatusColor(status.code)}`}>
              {status.code}
            </span>
            <div className="flex-1">
              <h3 className="text-sm font-semibold text-[var(--text)] mb-1">{status.status}</h3>
