  ...
  if (sourceFiles.length === 0) {
    return (
      <div className="h-full flex items-center justify-center">
        <p className="text-[var(--text-muted)]">No source code available</p>
      </div>
    );
  }
      ...
      <div className="flex border-b border-[var(--line)] bg-[var(--panel)]">
        <button
          ...
          className={`px-4 py-2 text-sm font-medium transition-colors focus:outline-none ${
            activeTab === 'files' 
              ? 'text-[var(--text)] bg-[var(--surface-2)] border-b-2 border-blue-500' 
      ...
      {activeTab === 'files' && (
        <>
          ...
          <div className="flex-grow overflow-auto">
            <div className="p-4">
              {sourceFiles.length > 1 && (
                <div className="mb-4 p-3 bg-[var(--surface-2)] rounded-lg border border-[var(--line)]">
                  <div className="text-sm text-[var(--text-muted)]">
                    <span className="text-[var(--text-muted)]">File:</span> {sourceFiles[activeFileIndex].path}
                  </div>
                </div>
              )}
      ...
      {activeTab === 'functions' && (
        <div className="flex-grow overflow-auto p-4 space-y-6">
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-[var(--text)]">Read Functions</h3>
            {readFunctions.length > 0 ? (
              <div className="space-y-3">
                {readFunctions.map((func, index) => {
                  ...
                  return (
                    <div key={index} className="bg-[var(--panel)] border border-[var(--line)] rounded-lg p-4 hover:border-[var(--line)] transition-colors">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="text-[var(--text)] font-mono text-sm font-semibold">
                            {func.name}()
                          </div>
                          {outputType && (
              ...
              <div className="text-[var(--text-muted)] text-sm">No read functions found</div>
            )}
          </div>

          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-[var(--text)]">Write Functions</h3>
            {writeFunctions.length > 0 ? (
              <div className="space-y-3">
                {writeFunctions.map((func, index) => (
                  <div key={index} className="bg-transparent border border-[var(--line)] rounded-lg p-4">
              ...
              <div className="text-[var(--text-muted)] text-sm">No write functions found</div>
            )}
          </div>
        </div>
      )}
          ...
          {hardcodedAddresses.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {hardcodedAddresses.map((addr, index) => (
                <div key={index} className="bg-[var(--panel)] rounded-lg border border-[var(--line)] p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <span className="text-blue-400 font-mono text-sm">{addr.address}</span>
                      {addr.contractName && (
                        <span className="px-2 py-1 bg-[var(--app-bg)] text-blue-300 text-xs rounded border border-[var(--line)]">
                          {addr.contractName}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="bg-[var(--panel)] rounded p-2 border border-[var(--line)]">
