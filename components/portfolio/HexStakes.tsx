...
import { IconRefresh, IconExternalLink, IconAlertTriangle, IconLock } from '@tabler/icons-react';
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-3 gap-2 sm:grid-cols-6">
        <Metric label="Staked" value={`${fmtHex(view.activePrincipal)}`} sub={view.usd(view.activePrincipal) != null ? fmtUsdShort(view.usd(view.activePrincipal)) : 'HEX'} />
...
function TabBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition-colors ${active ? 'bg-[var(--surface-2)] text-orange-300' : 'text-[var(--text-muted)] hover:text-[var(--text)]'}`}
    >
      {children}
    </button>
  );
...
function ActiveStakeCard({ stake, currentDay, hexUsd }: { stake: PulseChainHexStake; currentDay: number; hexUsd?: number | null }) {
  ...
  const barColor = currentDay < endDay ? 'var(--up)' : past <= 14 ? '#22c55e' : '#f59e0b';
    ...
    <div className="rounded-xl border border-[var(--line)] bg-[var(--surface)] p-4">
      ...
      <div className="mb-3 flex items-start justify-between gap-3">
        <div>
          ...
          <div className="text-lg font-semibold tabular-nums text-[var(--text)]">{fmtHex(principal)} HEX</div>
      ...
      <div className="mt-3 flex items-center justify-between text-[11px] text-[var(--text-muted)] tabular-nums">
        ...
        <a href={`${PLS_EXPLORER_TX}${stake.transactionHash}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-0.5 hover:text-[var(--text)]">
