...
const TransactionFlowDiagram: React.FC<TransactionFlowDiagramProps> = ({
  ...
  if (!transfers || transfers.length === 0) {
    return (
      <div className={`bg-[var(--panel)] rounded-lg p-4 border border-[var(--line)] ${className}`}>
        <p className="text-[var(--text-muted)] text-center">No transaction data available</p>
      </div>
    );
  }
  ...
  return (
    <div className={`bg-[var(--panel)] rounded-lg p-4 border border-[var(--line)] ${className}`}>
      <h3 className="text-lg font-semibold text-[var(--text)] mb-4">Transaction Flow - {tokenSymbol}</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        >
          ...
          <div className="text-xs text-[var(--text-muted)]">Total Transfers</div>
        </motion.div>
        ...
        >
          ...
          <div className="text-xs text-[var(--text-muted)]">Unique Addresses</div>
        </motion.div>
        ...
        >
          ...
          <div className="text-xs text-[var(--text-muted)]">Whale Moves</div>
        </motion.div>
      </div>

      {/* Flow Direction Chart */}
      <div className="mb-6">
        <h4 className="text-sm font-semibold text-[var(--text)] mb-3">Flow Direction</h4>
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-[var(--text-muted)]">Outflows</span>
      ...
      <div>
        <h4 className="text-sm font-semibold text-[var(--text)] mb-3">Recent Whale Movements</h4>
        <div className="space-y-2 max-h-48 overflow-y-auto">
            ...
            >
              <div className="flex-shrink-0">
                <div className="w-8 h-8 bg-[var(--app-bg)] rounded-full flex items-center justify-center">
