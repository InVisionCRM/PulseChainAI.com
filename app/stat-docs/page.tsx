      ...
      <div className="border-b border-[var(--line)] bg-[var(--app-bg)] backdrop-blur">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h1 className="text-4xl font-bold text-[var(--text)] mb-2">Stats API Documentation</h1>
            ...
            <a
              href="/admin-stats"
              className="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-[var(--text)] rounded-lg transition-colors"
            >
              Admin Panel
            </a>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <div className="flex-1">
              <input
                ...
                className="w-full px-4 py-3 bg-[var(--panel)] border border-[var(--line)] rounded-lg text-[var(--text)] placeholder-gray-500 focus:outline-none focus:border-purple-500"
              />
            </div>
            <select
              value={selectedFormat}
              onChange={(e) => setSelectedFormat(e.target.value)}
              className="px-4 py-3 bg-[var(--panel)] border border-[var(--line)] rounded-lg text-[var(--text)] focus:outline-none focus:border-purple-500"
            >
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        {filteredStats.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-[var(--text-muted)] text-lg mb-2">No stats found</p>
            <p className="text-[var(--text-muted)] text-sm">Try adjusting your search or filter</p>
          </div>
        ) : (
          <>
              ...
              {filteredStats.map(stat => {
                ...
                return (
                  <Link
                    ...
                    className="group bg-[var(--panel)] border border-[var(--line)] hover:border-purple-500/50 rounded-xl p-6 transition-all hover:shadow-lg hover:shadow-purple-500/10"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="text-lg font-semibold text-[var(--text)] group-hover:text-purple-400 transition-colors">
                        {stat.name}
                      </h3>
                    ...
                    <div className="mt-4 pt-4 border-t border-[var(--line)] flex items-center justify-between text-xs">
                      <span className="text-[var(--text-muted)]">View Documentation</span>
