...
const App: React.FC = () => {
  ...
  const sendMessage = useCallback(async (messageText: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/alex-rivera', {
        method: 'POST',
        headers,
        body: JSON.stringify({
          message: messageText,
        ...
        <div className="container mx-auto p-3 md:p-6 lg:p-8 max-w-4xl w-full pb-32 md:pb-16">
          ...
          <header className="flex items-center justify-between mb-4 md:mb-6 pb-3 md:pb-4 border-b border-slate-700">
            ...
            <div className="flex flex-col items-center">
              ...
              <p className="text-xs md:text-sm text-slate-400">Smart Contract Developer</p>
            </div>
            ...
            <div className="relative flex-grow bg-slate-800/30 backdrop-blur-sm rounded-xl border border-slate-700/50 overflow-hidden h-full">
              ...
              <div className="flex flex-col h-full">
                <div ref={chatContainerRef} className="flex-grow p-3 md:p-4 overflow-y-auto space-y-3 md:space-y-4 pb-20 md:pb-0">
                  {messages.length === 0 && (
                    <div className="text-center text-slate-400 h-full flex flex-col items-center justify-center gap-3 md:gap-4">
                      <div className="w-full max-w-md px-3 md:px-0">
                        <p className="mb-3 md:mb-4 text-sm md:text-base">Ask Alex Rivera about smart contract development...</p>
                        
                        {/* Question Templates */}
                        <div className="space-y-2">
                          <p className="text-xs text-slate-500 mb-2 md:mb-3">Development Questions:</p>
                          {[
                            ...
                            const colorClasses = [
                              ...
                              "bg-emerald-900/20 hover:bg-emerald-800/30 border-emerald-700/30 hover:border-emerald-600/40",
                  ...
                  {messages.map((msg) => (
                    <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] md:max-w-md lg:max-w-2xl rounded-xl px-3 md:px-4 py-2 md:py-3 ${msg.sender === 'user' ? 'bg-green-700 text-white' : 'bg-slate-700/80 backdrop-blur-sm text-slate-200 border border-slate-600/50'}`}>
                        {msg.sender === 'user' ? (
                ...
                <form onSubmit={handleSendMessage} className="p-3 md:p-4 border-t border-slate-700/50 flex items-center gap-2 md:gap-3 bg-slate-800/30 backdrop-blur-sm flex-shrink-0">
                  <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} placeholder="Ask about smart contract development..." className="flex-grow bg-slate-700 border border-slate-600 rounded-lg px-3 md:px-4 py-2 text-white placeholder-slate-400 focus:ring-2 focus:ring-green-500 focus:outline-none transition text-sm md:text-base" disabled={isLoadingChat} />
                  <button type="submit" disabled={isLoadingChat || !chatInput.trim()} className="bg-green-600 text-white p-2 rounded-full hover:bg-green-700 disabled:bg-slate-600 disabled:cursor-not-allowed transition-colors"><SendIcon className="w-4 h-4 md:w-5 md:h-5"/></button>
                </form>
              </div>
            </div>
          </div>
