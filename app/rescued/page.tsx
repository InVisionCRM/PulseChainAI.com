// The Rescue Wall — every HEX stake SuperStake has stopped from bleeding out.
//
...
import { WHAT_HAPPENED } from '@/lib/hex/rescueCopy';
...
export const metadata: Metadata = {
  ...
  description:
    'Every matured HEX stake SuperStake has frozen before the late-end penalty could eat it. Nothing taken, nothing given — the HEX is still the owner’s.',
};
    ...
    <div className="min-h-screen w-full bg-[var(--app-bg)]">
      <div className="mx-auto w-full max-w-5xl px-4 py-8 md:px-6 md:py-12">
        <div className="text-[11px] font-semibold uppercase tracking-wider text-emerald-400/90">
          a SuperStake public good · PulseChain
        </div>
        <h1 className="mt-2 text-3xl font-bold leading-tight text-[var(--text)] md:text-4xl">The Rescue Wall</h1>
        <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-[var(--text-muted)]">
          A matured HEX stake stops earning but does not stop losing — 1/700th a day until there is nothing
          left. Anyone can freeze that clock for anyone, and it pays the person who does it nothing. So we do
          it, for strangers, with our own gas. Every stake below is still its owner’s.
        </p>
        ...
        {rescues.length === 0 ? (
          ...
          <>
            <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <Big label="HEX still theirs" value={hex(t.claimableHex)} tone="good" />
              <Big
...
function Big({
  ...
  return (
    <div className="rounded-2xl border border-[var(--line)] bg-[var(--surface)] p-4">
      <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-[var(--text-faint)]">
        {icon}
        {label}
      </div>
