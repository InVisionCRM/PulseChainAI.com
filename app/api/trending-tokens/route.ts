...
export async function GET() {
  try {
    console.log('Starting trending tokens scrape...');
    
    // For now, let's fallback to the working API until we can debug the scraper
    console.log('Falling back to top-pairs API...');
    ...
    // Filter for PulseChain pairs and sort by volume (trending)
    const trendingPairs = allPairs
      ...
      ?.slice(0, 15) // Top 15 trending unique WPLS pairs
      ?.map((pair: any) => {
        const nonWPLSToken = pair.baseToken?.symbol === 'WPLS' ? pair.quoteToken : pair.baseToken;
        return {
    ...
    console.log(`Found ${allPairs.length} total pairs, ${trendingPairs.length} trending unique pairs after filtering`);
    
    return NextResponse.json({ pairs: trendingPairs });
  } catch (error) {
    console.error('Error fetching trending tokens:', error);
    return NextResponse.json(
      { error: 'Failed to fetch trending tokens', pairs: [] },
      { status: 500 }
    );
  }
}
