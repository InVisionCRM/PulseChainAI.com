...
export async function GET(req: NextRequest) {
  ...
  if (hit && Date.now() - hit.at < CACHE_TTL_MS) {
    return NextResponse.json(hit.value, { headers: { 'Cache-Control': 'public, max-age=300, s-maxage=300, stale-while-revalidate=1800' } });
  }
  ...
  try {
    ...
    return NextResponse.json(payload, { headers: { 'Cache-Control': 'public, max-age=300, s-maxage=300, stale-while-revalidate=1800' } });
  } catch (err) {
